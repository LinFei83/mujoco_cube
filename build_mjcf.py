...
def build() -> mjcf.RootElement:
    ...
    # Assets.
    # ================================ #
    root.asset.add("mesh", name="cubelet", file="cubelet.stl", scale="0.001 0.001 0.001")
    # Remove skybox texture as it's not supported in IsaacSim
    ...
    # Remove multi-color textures and materials - using single colors only
    # This solves the texture mapping issue with STL files
    # ================================ #
    ...
    for d1, d2 in list(itertools.combinations(axes, 2)):
        ...
        # Use the first color for edge pieces to avoid multi-color texture issues
        primary_color = dir2color[d1]
        body.add("geom", name=f"cubelet_{d}", material=primary_color, pos=dir2pos(d))
    ...
    for d1, d2, d3 in list(itertools.combinations(axes, 3)):
        ...
        body = core.add("body", name=d)
        body.add("joint", name=d)
        # Use the first available color for corner pieces
        primary_color = dir2color[d1]
        body.add("geom", name=f"cubelet_{d}", material=primary_color, pos=dir2pos(d))
    # ================================ #
...
def prettify_xml_string(xml_string: str) -> str:
    ...
    # Replace nasty filepaths for textures.
    texture_pattern = r"^[a-zA-Z]+(_[a-zA-Z]+)*-\w+\.png"
    textures = root.findall(".//texture")
    for texture in textures:
        for attr, file_name in texture.attrib.items():
            match = re.match(texture_pattern, file_name)
            if match:
                color = match.group(0).split("-")[0]
                texture.set(attr, f"{color}.png")
        # Keep texture names for IsaacSim compatibility
        # Do not remove texture names

    # Replace nasty filepaths for meshes.
    mesh_pattern = r"^cubelet-\w+\.stl$"
    meshes = root.findall(".//mesh")
    for mesh in meshes:
        for attr, file_name in mesh.attrib.items():
            if attr == "file" and re.match(mesh_pattern, file_name):
                mesh.set(attr, "cubelet.stl")
